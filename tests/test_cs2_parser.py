...
class _VendorParser:
    ...
    def parse_header(self) -> dict[str, object]:
        return {
            "map_name": "de_dust2",
            "server_name": "ClutchIQ Test Server",
            "client_name": "clutchiq",
        }

    def parse_player_info(self) -> _FakeFrame:
        return _FakeFrame(
            [
                {"id": 1, "name": "alice", "team": "T"},
                {"id": 2, "name": "bob", "team": "CT"},
            ]
        )
    ...
    def parse_events(self, event_names: list[str], player=None, other=None):
        ...
        return [
            ("round_start", _FakeFrame([{"tick": 1, "round": 1}])),
            ("round_end", _FakeFrame([{"tick": 120, "round": 1, "winner_team": "T"}])),
        ]
