...
class DemoHistoryService:
    ...
    def load_summary(self) -> DashboardSummary:
        try:
            ...
            return DashboardSummary(False, True, "Import history unavailable", 0, 0, "Unavailable", (), ())
        ...
        recent_demos = tuple(
            RecentDemoEntry(record.id, record.source_name, record.imported_at_utc, record.result, record.parse_stage,
                            record.analysis_summary.winning_side if record.analysis_summary is not None else None)
            for record in records[-5:]
        )
        latest = records[-1] if records else None
        import_status = "No imports yet" if latest is None else (
            f"Last import succeeded: {latest.source_name}" if latest.result == ImportResult.SUCCESS
            else f"Last import failed: {latest.source_name}"
        )
        return DashboardSummary(True, not records, import_status, total_demos_imported, total_demos_imported,
                                latest.imported_at_utc if latest is not None else "No imports yet", recent_demos, records)
    ...
    def _decode_record(self, raw: object) -> PersistedImportRecord:
        ...
        if isinstance(analysis_raw, dict):
            analysis = AnalysisSummary(
                total_rounds=int(analysis_raw["total_rounds"]), ct_rounds=int(analysis_raw["ct_rounds"]),
                t_rounds=int(analysis_raw["t_rounds"]), winning_side=str(analysis_raw["winning_side"]),
                rounds_with_known_winner=int(analysis_raw["rounds_with_known_winner"]),
                map_name=str(analysis_raw["map_name"]) if analysis_raw.get("map_name") is not None else None,
            )
        return PersistedImportRecord(str(raw["id"]), str(raw["imported_at_utc"]),
                                     str(raw["source_path"]) if raw.get("source_path") is not None else None,
                                     str(raw["source_name"]), ImportResult(str(raw["result"])),
                                     ImportStage(str(raw["parse_stage"])), analysis,
                                     str(raw["error_type"]) if raw.get("error_type") is not None else None,
                                     str(raw["error_message"]) if raw.get("error_message") is not None else None)

    def _encode_result(self, result: DemoImportResult) -> dict:
        return {"id": result.id, "imported_at_utc": result.imported_at_utc.isoformat(),
                "source_path": str(result.source_path) if result.source_path is not None else None,
                "source_name": result.source_name, "result": result.result.value,
